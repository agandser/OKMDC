...
def main():
    ...
    ax1.set(
        xlim=[-8, 8],
        ylim=[-8, 8],
        title=f"V - зеленый вектор (масштаб {V_scale})\nW - красный вектор (масштаб {W_scale})",
    )
    ax1.plot(X, Y)
    ...
    X_arr_W = np.multiply(W_scale, np.array([-7, 0, -7])) # X координаты трех точек стрелки вектора W
    Y_arr_W = np.multiply(W_scale, np.array([2, 0, -2])) # Y координаты трех точек стрелки вектора W
    ...
    def animate(i):
        ...
        return point, V_line, V_arrow, W_line, W_arrow

    animation = FuncAnimation(fig, animate, frames=time_steps_amount, interval=25)
